...
public class NavigationFactory : NSObject, FlutterStreamHandler, NavigationViewControllerDelegate {
    ...
    func startNavigationWithWayPoints(wayPoints: [Waypoint], flutterResult: @escaping FlutterResult) {
        ...
        Directions.shared.calculate(options) { [weak self](session, result) in
            ...
            case .failure(let error):
                strongSelf.sendEvent(eventType: RouteEventType.route_build_failed)
    ...
    func continueNavigationWithWayPoints(wayPoints: [Waypoint]) {
        ...
        Directions.shared.calculate(_options!) { [weak self](session, result) in
            ...
            switch result {
            case .failure(let error):
                strongSelf.sendEvent(eventType: RouteEventType.route_build_failed, data: error.localizedDescription)
            case .success(let response):
                strongSelf.sendEvent(eventType: RouteEventType.route_built)
                guard let routes = response.routes else { return }
                //TODO: if more than one route found, give user option to select one: DOES NOT WORK
                if(routes.count > 1 && strongSelf.ALLOW_ROUTE_SELECTION){
                    //show map to select a specific route
    ...
    func endNavigation(result: FlutterResult?) {
        sendEvent(eventType: RouteEventType.navigation_finished)
        if(self._navigationViewController != nil) {
    ...
    func sendEvent(eventType: RouteEventType, data: String = "") {
        let routeEvent = RouteEvent(eventType: eventType, data: data)
    ...
    public func navigationViewController(_ navigationViewController: NavigationViewController, didUpdate progress: RouteProgress, with location: CLLocation, rawLocation: CLLocation) {
        ...
        sendEvent(eventType: RouteEventType.navigation_running)
    ...
    public func navigationViewController(_ navigationViewController: NavigationViewController, didArriveAt waypoint: Waypoint) -> Bool {
        ...
        sendEvent(eventType: RouteEventType.on_arrival, data: "true")
    ...
    public func navigationViewControllerDidDismiss(_ navigationViewController: NavigationViewController, byCanceling canceled: Bool) {
        if(canceled) {
            sendEvent(eventType: RouteEventType.navigation_cancelled)
        }
